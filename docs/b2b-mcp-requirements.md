...
|---|---|---|---|---|
| 1 | **Company, product & order reads** first | ✅ | `main` — read tools | `search_products`, `get_product`, `check_stock`, `browse_categories`, `get_category_products`, `get_customer`, `get_order_status`, `get_my_company`. Verified end-to-end. |
...
| 3 | **Authenticated account binding** (before placement) | 🔩 | `branch` — `src/auth` | Scoped `ActorContext` + `SessionStore` (expiry, revocation) + `AuthProvider` seam are built. The real binding — `DelegatedAuthProvider` + a `TokenVerifier` (OAuth 2.1 + PKCE, or whatever the group picks) — is a stub that refuses until wired. Current live `login` is a dev-only resource-owner flow to **replace**. |
...
1. **Wire `src/auth/*` into the live server** (`index.ts`) behind a feature flag:
   retire the `login` tool, re-tier cart/checkout tools, and route placement
   through `requestPlaceOrder` / `getOperationStatus`.
